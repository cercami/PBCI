...
list_time_ad_cca = []  # list to store the time per trial
...
for s in range(0, Ns):
    ...
    list_time_ad_cca.append(mat_time)  # store results per subject
...
mat_time_ad_cca = np.concatenate(list_time_ad_cca, axis=1)
...
np.save(os.path.join(dir_results, 'mat_result_ad_cca'), mat_result_ad_cca)
np.save(os.path.join(dir_results, 'mat_time_ad_cca'), mat_time_ad_cca)