...
def apply_advanced_cca(X, Y, X_Train):
    ...
    cca5.fit(X.transpose(), X_Train.transpose())
    w_x = cca5.x_weights_
    w_xt = cca5.y_weights_
    ...
    rho_4 = np.diag(
        ...
        n_comp:])
    rho_5 = np.diag(
        np.corrcoef(np.matmul(X_Train.transpose(), w_x), np.matmul(X_Train.transpose(), w_xt), rowvar=False)[:n_comp,
        n_comp:])
...
for s in range(0, Ns):
    ...
    for b in range(0, Nb):
        ...
        for f in range(0, Nf):
            ...
            num_iter = num_iter + 1
            print("Advanced FBCCA: Iteration " + str(num_iter) + " of " + str(Nf * Nb * Ns), flush=True)
    ...
    list_time_fbcca.append(mat_time)  # store time per subject

    t_end = datetime.now()
    print("Advanced FBCCA: Elapsed time for subject: " + str(s + 1) + ": " + str((t_end - t_start)), flush=True)

mat_result_ad_fbcca = np.concatenate(list_result_fbcca, axis=1)
mat_time_ad_fbcca = np.concatenate(list_time_fbcca, axis=1)

### analysis
gof_ad_fbcca = gof(vec_freq, mat_result_ad_fbcca)
accuracy_ad_fbcca = accuracy(vec_freq, mat_result_ad_fbcca)

print("Advanced FBCCA: gof: " + str(gof_ad_fbcca))
print("Advanced FBCCA: accuracy: " + str(accuracy_ad_fbcca))

plt.figure()
plt.imshow(mat_result_ad_fbcca)

np.save(os.path.join(dir_results, 'mat_result_ad_fbcca'), mat_result_ad_fbcca)
np.save(os.path.join(dir_results, 'mat_time_ad_fbcca'), mat_time_ad_fbcca)